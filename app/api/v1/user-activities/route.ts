...
export async function GET(request: NextRequest) {
  try {
    ...
    // 필터 파라미터
    if (typeParam) {
      const validTypes = ['note', 'quote', 'reading', 'finished'];
    ...
    // 백엔드 API 호출
    const upstreamUrl = `${PUBLIC_API_BASE_URL}/api/v1/user-activities?${queryParams.toString()}`;
    ...
    console.error('전체 활동 API 호출 오류:', error);
